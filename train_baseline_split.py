...
#    X_train=np.load('./data_word2vec_feature/train_vector_lower.npz')
#    X_test=np.load('./data_word2vec_feature/test_vector_lower.npz')
#    
#    X2=np.reshape(np.array(X_train['train_data'][:-50000])[:,6],(-1,1))  
#    X_val_2=np.reshape(np.array(X_train['train_data'][-50000:])[:,6],(-1,1))  
#    X_test_2=np.reshape(np.array(X_test['test_data'])[:,6],(-1,1))  
#    
#    X=np.concatenate((X,X2),axis=1)
#    X_val_=np.concatenate((X_val_,X_val_2),axis=1)
#    X_test_=np.concatenate((X_test_,X_test_2),axis=1)
    
#    X=np.delete(X,[14,17,20,21,22,23],axis=1)
#    X_val_=np.delete(X_val_,[14,17,20,21,22,23],axis=1)
#    X_test_=np.delete(X_test_,[14,17,20,21,22,23],axis=1)
...
#           u'prediction9', u'similarity9',u'equal_rate9', u'prefix_title_sim']
    ...
    for i in [0,1,2,26,27,28,7,10,9,11,19]:
    ...
    X3=skb.fit_transform(X3, y)
    X_val_3=X_val_3[:,skb.get_support(True)]
    X_test_3=X_test_3[:,skb.get_support(True)]
...
#    val_nodata_save.to_csv('./submit/split_data/val_nodata_save.csv',index = False)